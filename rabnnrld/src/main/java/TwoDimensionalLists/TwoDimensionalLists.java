...
public class TwoDimensionalLists implements ITwoDimensionalLists{
    ...
    //AUTOMATICO
    public TwoDimensionalLists(String text, char activate, char inactive){
        this.activate = activate; this.inactive = inactive; 
        nf= 8;
        twoDimensionallist  = new Aleatoria();
        if(text.contains("figures:")){
            text = text.replace("figures:", "");
            text = Figures.DrawFigure(text);
            DrawItem(text, false, 0);
        }else if(text.contains("text:")){
            text = text.replace("text:", "");
            char[] caracteres = text.toCharArray(); int k = 0;
            for(int i = 0; i < caracteres.length; i++){
                DrawCharacters(false, caracteres[i], k);
                k+=itemWidth;
            }
        }
        nc = initialSize = ((Aleatoria)twoDimensionallist.get(0)).length();
        isDefined = false;
    }
    ...
    public void setText(String text){
        if(text.contains("figures:")){
            text = text.replace("figures:", "");
            text = Figures.DrawFigure(text);
            DrawItem(text, isDefined, 0);
        }else if(text.contains("text:")){
            text = text.replace("text:", "");
            char[] caracteres = text.toCharArray(); int k = 0;
            for(int i = 0; i < caracteres.length; i++){
                DrawCharacters(isDefined, caracteres[i], k);
                k+=itemWidth;
            }
        }
        initialSize = ((Aleatoria)twoDimensionallist.get(0)).length();
    }
    ...
    private void DrawCharacters(boolean reemplace, char character, int c){
       String codeCharacter = "";
       if(Character.isLetter(character)){
            codeCharacter = Alphabet.DrawLetter(character);
       }else if(Character.isDigit(character)){
            codeCharacter = Numbers.DrawNumber(character);
       }else{
            codeCharacter = Symbols.DrawSymbol(character);
       }
        DrawItem(codeCharacter, reemplace, c);
    }
    ...
    private void DrawItem(String code, boolean reemplace, int columns){
        String[] rows = code.split("-");
        itemWidth = rows[0].length();
        
        if(reemplace == false){
            for(int i = 0; i < rows.length; i++){
                Lista row; int k = 0;
                if(columns == 0){ row = new Aleatoria();}
                else{ row = (Aleatoria)twoDimensionallist.get(i); }
                for(int j = columns; j<columns + rows[i].length(); j++){
                    row.add(j, rows[i].charAt(k)); k++;
                }
                if(columns == 0){twoDimensionallist.add(i, row);}
            }
        }
        else{
            for(int i = 0; i < rows.length; i++){
